...
pub fn process_create_metadata_accounts_logic(
    ...
    //     metadata.subscription = subscription.unwrap();
    // } else {
    //     metadata.subscription = Pubkey::new_from_array([0; 32]);
    //     // metadata.subscription = None;
    //     msg!("Set empty subscription: {:?}", metadata.subscription);
    // }
...
pub fn process_mint_new_edition_from_master_edition_via_token_logic<'a>(
    ...
    msg!("Mint limited edition with subscription {:?}", subscription);
