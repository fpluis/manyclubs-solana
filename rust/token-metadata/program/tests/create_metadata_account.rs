...
async fn success() {
    ...
    let subscription_key = Pubkey::new_unique();
    ...
    // Weird bug with encoding and the trailing null character
    // assert_eq!(metadata.data.name, "Test".to_string());
    // assert_eq!(metadata.data.symbol, "TST".to_string());
    // assert_eq!(metadata.data.uri, "uri".to_string());
    assert_eq!(metadata.data.seller_fee_basis_points, 10);
    assert_eq!(metadata.data.creators, None);
    assert_eq!(metadata.subscription, Some(subscription_key));
