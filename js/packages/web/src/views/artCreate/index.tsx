...
export const ArtCreateView = () => {
  ...
  // store files
  const mint = async () => {
    console.log(`Before mint; attributes: ${JSON.stringify(attributes)}`);
    const metadata = {
...
const subscriptionSelectAfter = props => (
  <Select
    onClick={event => {
      // Prevent some weird behavior where the focus shifts
      event.preventDefault();
    }}
    onChange={(value: any) => {
      console.log(
        `Setting period type; subscriptionConfig: ${JSON.stringify(
    ...
    <>
      ...
      <Row className="content-action" justify="space-around">
        ...
        <Col className="section" style={{ minWidth: 300 }}>
          ...
          <label className="action-field">
            <span className="field-title">Subscription</span>
            <Row>
              <span>Pay</span>
            </Row>
            <Row>
              <Col xs={24}>
                <InputNumber
                  ...
                  onChange={(val: number) => {
                    console.log(
                      `Setting price; subscriptionConfig: ${JSON.stringify(
                        props.attributes.subscriptionConfig,
                      )}`,
                    );
                    props.setAttributes({
            ...
            <Row>
              <Col xs={24}>
                <InputNumber
                  ...
                  addonAfter={subscriptionSelectAfter(props)}
                  onChange={(val: number) => {
                    console.log(
                      `Setting period amount; subscriptionConfig: ${JSON.stringify(
                        props.attributes.subscriptionConfig,
                      )}`,
                    );
                    props.setAttributes({
