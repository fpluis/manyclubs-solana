...
// Empty 32-Length u8 array in base58
...
export class Metadata {
  ...
  subscription: StringPublicKey | null;
...
class MintNewEditionFromMasterEditionArgs {
  instruction: number = 11;
  edition: BN;
  subscription: StringPublicKey | null;

  constructor(args: { edition: BN; subscription: StringPublicKey | null }) {
    this.edition = args.edition;
    this.subscription = args.subscription;
  }
}
...
export const METADATA_SCHEMA = new Map<any, any>([
  ...
  [
    MintNewEditionFromMasterEditionArgs,
    {
      kind: 'struct',
      fields: [
        ['instruction', 'u8'],
...
export async function createMetadata(
  ...
  console.log(
    `Serialize metadata value ${JSON.stringify(value)}: ${JSON.stringify(
      Buffer.from(serialize(METADATA_SCHEMA, value)),
    )}`,
  );
  const txnData = Buffer.from(serialize(METADATA_SCHEMA, value));
...
export async function mintNewEditionFromMasterEditionViaToken(
  ...
  const data = Buffer.from(
    serialize(
      METADATA_SCHEMA,
      new MintNewEditionFromMasterEditionArgs({ edition, subscription }),
    ),
  );
  console.log(`Mint new edition with subscription ${subscription}`);
